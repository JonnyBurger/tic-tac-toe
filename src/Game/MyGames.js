    ...
    this.props.contract.methods
      ...
      .then(res => {
        for (let i = 0; i < res.gameIds.length; i++) {
          ...
          if (owner == this.props.account.ethAddress
            || playerX == this.props.account.ethAddress || playerO == this.props.account.ethAddress) {
            let game = {
              id: res.gameIds[i],
                ...
                {this.state.games.map(game => (
                  <tr key={game.id}>
                    ...
                    <td style={{width: 120}}>
                      ...
                      ) : (game.status == 'X_HAS_TURN' || game.status == 'O_HAS_TURN') ? (
                        <PlayGameButton>
                          <GameIcon
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 640 512"
                          ...
                          <JoinParagraph>Play</JoinParagraph>
                        </PlayGameButton>
                      ) : (<StartGameButton/>)}
                    </td>
                  </tr>
